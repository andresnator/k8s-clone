...
describe('Migrator', () => {
    ...
    beforeEach(() => {
        ...
        sourceClient.getCoreApi = jest.fn().mockReturnValue(sourceCoreApi);
        sourceClient.getAppsApi = jest.fn().mockReturnValue(sourceAppsApi);
        destClient.getCoreApi = jest.fn().mockReturnValue(destCoreApi);
        destClient.getAppsApi = jest.fn().mockReturnValue(destAppsApi);
        sourceClient.getCurrentContext = jest.fn().mockReturnValue('source-context');
        destClient.getCurrentContext = jest.fn().mockReturnValue('dest-context');
        ui.logInfo = jest.fn();
    ...
    describe('migrateResources', () => {
        ...
        it('should handle AlreadyExists error gracefully', async () => {
            ...
            expect(ui.logError).toHaveBeenCalledWith(expect.stringContaining("'my-configmap' already exists"));
        });
        it('should handle general API errors gracefully', async () => {
            ...
            expect(ui.logError).toHaveBeenCalledWith(expect.stringContaining('Failed to migrate ConfigMap my-configmap'));
        });
